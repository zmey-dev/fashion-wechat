...
Page({
  ...
  getUserInfo() {
    ...
    if (userInfo && userInfo.university) {
      this.setData({
        universityName: userInfo.university.name || userInfo.university || '北京大学'
      });
    }
    ...
    this.loadEventFromAPI(eventId, {
      success: (eventData) => {
        ...
        this.setData({
          ...
          'calendarData.calendarDates': this.generateCalendarDates(startDate.getFullYear(), startDate.getMonth() + 1)
        });
  ...
  prevMonth() {
    ...
    this.setData({
      ...
      'calendarData.calendarDates': this.generateCalendarDates(currentYear, currentMonth)
    });
  },
  ...
  initCalendar() {
    const today = new Date();
    const calendarDates = this.generateCalendarDates(today.getFullYear(), today.getMonth() + 1);
  ...
  // Form validation
  validateForm() {
    const { title, startDate, endDate, allowLimit, limit, description, posterImage, posterImageUrl, eventId } = this.data;
    ...
    wx.request({
      ...
      success: (res) => {
        if (res.statusCode === 200 || res.statusCode === 201) {
          ...
          if (callbacks.fail) {
            callbacks.fail(new Error(isUpdate ? 'Failed to update event' : 'Failed to create event'));
          }
        }
      },
      fail: (error) => {
